...
const shakeItems = menu.filter((item) => item.category === 'shakes');

window.addEventListener('DOMContentLoaded', function() {
  displayMenuItems(menu);
  filterBtns.forEach((btn) => {
    btn.addEventListener('click', () => {
      if (btn.textContent === 'breakfast') {
        displayMenuItems(breakfastItems);
      } else if (btn.textContent === 'lunch') {
        displayMenuItems(lunchItems);
      } else if (btn.textContent === 'shakes') {
        displayMenuItems(shakeItems);
      }
    });
  });
});
